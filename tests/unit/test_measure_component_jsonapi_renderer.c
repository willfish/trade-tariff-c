...
static void renders_component_resource_from_typed_model(void **state)
{
    (void)state;

    MeasureComponentModel model = component("123", "01");
    yyjson_mut_doc *mut_doc = yyjson_mut_doc_new(NULL);
    assert_non_null(mut_doc);
    yyjson_mut_val *resource = measure_component_jsonapi_resource(mut_doc, &model);
    ...
    assert_string_equal(string_at(root, "id", NULL), "123-01");
    assert_string_equal(string_at(root, "type", NULL), "measure_component");
    ...
    assert_string_equal(string_at(obj_path(yyjson_obj_get(relationships, "measurement_unit"), "data", NULL), "id", NULL), "KGM");
    assert_string_equal(string_at(obj_path(yyjson_obj_get(relationships, "measurement_unit_qualifier"), "data", NULL), "id", NULL), "X");
...
static void renders_relationship_refs_for_one_measure(void **state)
{
    ...
    assert_true(measure_component_model_list_append(&components, component("456", "01")));
    assert_true(measure_component_model_list_append(&components, component("123", "99")));

    yyjson_mut_doc *mut_doc = yyjson_mut_doc_new(NULL);
    assert_non_null(mut_doc);
    yyjson_mut_val *data = measure_component_jsonapi_relationship_data_for_measure(mut_doc, &components, "123");
    ...
    assert_string_equal(string_at(yyjson_arr_get(root, 0), "id", NULL), "123-01");
    assert_string_equal(string_at(yyjson_arr_get(root, 1), "id", NULL), "123-99");
...
int main(void)
{
    const struct CMUnitTest tests[] = {
        cmocka_unit_test(renders_component_resource_from_typed_model),
