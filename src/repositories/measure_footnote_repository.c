...
#define MEASURE_FOOTNOTE_REPOSITORY_COLS 4
...
static char *build_sql(const CommodityRequestContext *context)
{
    ...
    char *template = sql_loader_load("sql/measure_footnotes/for_measure_sids.sql");
...
int measure_footnote_repository_hydrate_row(const DbRows *rows,
                                            ...
                                            Footnote *footnote)
{
    if (rows == NULL || footnote == NULL || row < 0 || row >= rows->rows || rows->cols < MEASURE_FOOTNOTE_REPOSITORY_COLS) {
        return 0;
    }

    *footnote = (Footnote){
        .measure_sid = owned_value(rows, row, 0),
        .id = owned_value(rows, row, 1),
        .code = owned_value(rows, row, 2),
        .description = owned_value(rows, row, 3),
    };

    if (footnote->measure_sid == NULL ||
        footnote->id == NULL ||
        footnote->code == NULL) {
                                                       ...
                                                       FootnoteList *footnotes)
{
    ...
    char *sid_param = measure_footnote_repository_sid_array_param(measure_sids, sid_count);
    ...
    DbRows rows = db_fetch_rows("measure_footnotes_for_measure_sids", sql, sid_param);
    free(sid_param);
    free(sql);
    if (!rows.ok) {
        db_rows_free(&rows);
    ...
    for (int row = 0; row < rows.rows; row++) {
        Footnote footnote = { 0 };
        if (!measure_footnote_repository_hydrate_row(&rows, row, &footnote)) {
            footnote_list_free(footnotes);
            db_rows_free(&rows);
            return 0;
        }
        if (!footnote_list_append(footnotes, footnote)) {
            footnote_list_free(footnotes);
            db_rows_free(&rows);
            return 0;
        }
