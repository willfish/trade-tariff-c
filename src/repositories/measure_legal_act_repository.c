...
#define MEASURE_LEGAL_ACT_REPOSITORY_COLS 13
...
char *measure_legal_act_repository_sid_array_param(const char *const *measure_sids,
                                                   size_t sid_count)
{
...
static char *build_sql(const CommodityRequestContext *context)
{
    ...
    char *template = sql_loader_load("sql/measure_legal_acts/for_measure_sids.sql");
...
int measure_legal_act_repository_hydrate_row(const DbRows *rows,
                                             int row,
                                             MeasureLegalActModel *model)
{
    if (rows == NULL || model == NULL || row < 0 || row >= rows->rows || rows->cols < MEASURE_LEGAL_ACT_REPOSITORY_COLS) {
        return 0;
    }

    *model = (MeasureLegalActModel){
        .measure_sid = owned_value(rows, row, 0),
        .measure_type_id = owned_value(rows, row, 1),
        .regulation_id = owned_value(rows, row, 2),
        .role = owned_value(rows, row, 3),
        .source = owned_value(rows, row, 4),
        .validity_start_date = owned_value(rows, row, 5),
        .validity_end_date = owned_value(rows, row, 6),
        .effective_start_date = owned_value(rows, row, 7),
        .effective_end_date = owned_value(rows, row, 8),
        .officialjournal_number = owned_value(rows, row, 9),
        .officialjournal_page = owned_value(rows, row, 10),
        .published_date = owned_value(rows, row, 11),
        .information_text = owned_value(rows, row, 12),
    };
...
int measure_legal_act_repository_fetch_for_measure_sids(const CommodityRequestContext *context,
                                                        const char *const *measure_sids,
                                                        size_t sid_count,
                                                        MeasureLegalActModelList *legal_acts)
{
    if (context == NULL || legal_acts == NULL) {
        return 0;
    }

    char *sid_param = measure_legal_act_repository_sid_array_param(measure_sids, sid_count);
    ...
    DbRows rows = db_fetch_rows("measure_legal_acts_for_measure_sids", sql, sid_param);
    free(sid_param);
    free(sql);
    if (!rows.ok) {
        db_rows_free(&rows);
    ...
    for (int row = 0; row < rows.rows; row++) {
        ...
        if (!measure_legal_act_repository_hydrate_row(&rows, row, &model)) {
            measure_legal_act_model_list_free(legal_acts);
            db_rows_free(&rows);
            return 0;
        }
        if (!measure_legal_act_model_list_append(legal_acts, model)) {
            measure_legal_act_model_list_free(legal_acts);
            db_rows_free(&rows);
            return 0;
        }
...
int measure_legal_act_repository_self_test(void)
{
