...
#define QUOTA_EVENT_REPOSITORY_COLS 3
...
static char *build_sql(const CommodityRequestContext *context)
{
    ...
    char *template = sql_loader_load("sql/quota_events/latest_for_definition_sids.sql");
...
{
    if (rows == NULL || model == NULL || row < 0 || row >= rows->rows || rows->cols < QUOTA_EVENT_REPOSITORY_COLS) {
        return 0;
    }

    *model = (QuotaEventModel){
        .quota_definition_sid = owned_value(rows, row, 0),
        .occurrence_timestamp = owned_value(rows, row, 1),
        .event_type = owned_value(rows, row, 2),
    };

    if (model->quota_definition_sid == NULL ||
        model->occurrence_timestamp == NULL ||
        model->event_type == NULL) {
                                                            ...
                                                            QuotaEventModelList *events)
{
    ...
    char *sid_param = quota_event_repository_text_array_param(quota_definition_sids, quota_definition_sid_count);
    ...
    DbRows rows = db_fetch_rows("quota_events_latest_for_definition_sids", sql, sid_param);
    free(sid_param);
    free(sql);
    if (!rows.ok) {
        db_rows_free(&rows);
    ...
    for (int row = 0; row < rows.rows; row++) {
        ...
        if (!quota_event_repository_hydrate_row(&rows, row, &model)) {
            quota_event_model_list_free(events);
            db_rows_free(&rows);
            return 0;
        }
        if (!quota_event_model_list_append(events, model)) {
            quota_event_model_list_free(events);
            db_rows_free(&rows);
            return 0;
        }
