...
#define MEASURE_EXCLUDED_GEOGRAPHICAL_AREA_REPOSITORY_COLS 3
...
static char *build_sql(const CommodityRequestContext *context)
{
    ...
    char *template = sql_loader_load("sql/measure_excluded_geographical_areas/for_measure_sids.sql");
...
{
    if (rows == NULL || model == NULL || row < 0 || row >= rows->rows || rows->cols < MEASURE_EXCLUDED_GEOGRAPHICAL_AREA_REPOSITORY_COLS) {
        return 0;
    }

    *model = (MeasureExcludedGeographicalAreaModel){
        .measure_sid = owned_value(rows, row, 0),
        .geographical_area_sid = owned_value(rows, row, 1),
        .geographical_area_id = owned_value(rows, row, 2),
    };

    if (model->measure_sid == NULL || model->geographical_area_id == NULL) {
...
{
    if (context == NULL || excluded == NULL) {
        return 0;
    }

    char *sid_param = measure_excluded_geographical_area_repository_sid_array_param(measure_sids, sid_count);
    ...
    DbRows rows = db_fetch_rows("measure_excluded_geographical_areas_for_measure_sids", sql, sid_param);
    free(sid_param);
    free(sql);
    if (!rows.ok) {
        db_rows_free(&rows);
    ...
    for (int row = 0; row < rows.rows; row++) {
        ...
        if (!measure_excluded_geographical_area_repository_hydrate_row(&rows, row, &model)) {
            measure_excluded_geographical_area_model_list_free(excluded);
            db_rows_free(&rows);
            return 0;
        }
        if (!measure_excluded_geographical_area_model_list_append(excluded, model)) {
            measure_excluded_geographical_area_model_list_free(excluded);
            db_rows_free(&rows);
            return 0;
        }
